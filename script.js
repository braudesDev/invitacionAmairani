...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!audio || !botonReproducir || !botonSiguiente) {
      console.error("❌ Error: No se encontró uno de los elementos del reproductor.");
      return;
  }
  ...
  function nextSong() {
      ...
      audio.play()  // Reproduce la nueva canción
          ...
          .catch(error => console.error("❌ Error al cambiar de canción:", error));
  }
...
// Fecha de la fiesta
const fechaFiesta = new Date("2025-03-06T00:44:00").getTime();
...
document.getElementById("uploadButton").addEventListener("click", async (event) => {
    ...
    if (fileInput.files.length > 5) {
        statusMessage.textContent = "¡Recuerda que solo puedes subir un máximo de 5 archivos a la vez! :)"; // Usamos "statusMessage"
        statusMessage.style.display = "block"; // Mostrar el mensaje de error
        return;
    }
    ...
    if (totalSize > maxSize) {
        statusMessage.textContent = "¡El tamaño total de los archivos no debe superar los 50 MB!"; // Usamos "statusMessage"
        statusMessage.style.display = "block"; // Mostrar el mensaje de error
        return;
    }
